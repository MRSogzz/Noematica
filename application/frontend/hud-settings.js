...
loadQuests();
// 開機自動套用風格檔案（assets/themes/<id>/theme.json）。預設主題是
// noematica（手繪暖色風格，見 assets/themes/noematica/theme.json）；
// 想換成別的主題，把 'noematica' 換成 assets/themes/ 底下其他資料夾
// 名稱即可（目前還有 default、cyber、gamification）；silent:true 是
// 因為這是開機自動觸發，theme.json 萬一還沒準備好不該跳錯誤 toast
// 打擾使用者，手動從設定面板上傳 theme.json（upTheme()）失敗時才需要
// 提示。
loadThemeFromServer('noematica', {silent: true});