...
PANELS.m = async function m() {
    ...
    // ── 樣式注入：先拆掉舊的，保證每次進面板都是最新的 ──
    document.getElementById('wiki-ai-style')?.remove();
    {
      const s = document.createElement('style');
      s.id = 'wiki-ai-style';
      s.textContent = `
        /* 面板根：強制所有子元素繼承深色文字，並蓋掉外層任何 color 設定 */
        .wiki-ai-panel,
        .wiki-ai-panel * {
        ...
        .wiki-ai-header {
          ...
          border-bottom: 1px solid rgba(120,84,40,0.25);
          flex-shrink: 0;
        }
        .wiki-ai-title {
        ...
        .wiki-ai-msg.user {
          align-self: flex-end;
          background: rgba(184,155,107,0.28);
        ...
        .wiki-ai-msg.ai.question { border-color: rgba(180,110,10,0.75); }
        .wiki-ai-msg.ai.files    { border-color: rgba(30,120,70,0.7); }
        .wiki-ai-msg.system {
        ...
        .wiki-ai-file-item {
          background: rgba(30,120,70,0.16);
          border: 1px solid rgba(30,120,70,0.5);
          border-radius: 5px; padding: 6px 10px; font-size: 10px;
          color: #0E4A2A !important;
        ...
        .wiki-ai-btn.confirm {
          color: #0E4A2A !important;
          border-color: rgba(30,120,70,0.75);
          background: rgba(30,120,70,0.16);
        }
        .wiki-ai-btn.confirm:hover { background: rgba(30,120,70,0.28); }
        .wiki-ai-btn.cancel {
          color: #1F1A14 !important;
          border-color: rgba(120,84,40,0.55);
          background: rgba(120,84,40,0.1);
        }
        .wiki-ai-btn.cancel:hover { background: rgba(120,84,40,0.22); }
        .wiki-ai-input-row {
          display: flex; gap: 8px; padding: 10px 14px 12px;
          border-top: 1px solid rgba(120,84,40,0.25); flex-shrink: 0;
        }
        .wiki-ai-input {
          flex: 1; background: #FFFFFF;
          border: 1px solid rgba(120,84,40,0.45); border-radius: 8px;
        ...
        .wiki-ai-input:focus { border-color: #6B4F1F; }
        .wiki-ai-input::placeholder { color: #6B5D4F !important; opacity: 1 !important; }
        .wiki-ai-send {
          width: 36px; height: 36px; border-radius: 8px; flex-shrink: 0;
          background: rgba(184,155,107,0.28);
          border: 1px solid rgba(150,120,60,0.75);
          color: #6B4F1F !important;
          font-size: 16px; cursor: pointer; display: flex;
          align-items: center; justify-content: center; transition: background .12s;
          align-self: flex-end;
        }
        .wiki-ai-send:hover { background: rgba(184,155,107,0.42); }
        .wiki-ai-send:disabled { opacity: .55; cursor: not-allowed; }
        .wiki-doc-item {
          display: flex; align-items: center; gap: 10px; padding: 9px 14px;
          cursor: pointer; border-radius: 6px; margin-bottom: 2px;
          transition: background .12s;
        }
        .wiki-doc-item:hover { background: rgba(184,155,107,0.18); }
        .wiki-doc-item.active {
          background: rgba(184,155,107,0.26);
          border-left: 2px solid #6B4F1F; padding-left: 12px;
        }
        ...
        .wiki-empty-ai-icon { font-size: 36px; opacity: .9; }
        .wiki-empty-ai-text {
          font-size: 11px;
          color: #3A3228 !important;
    ...
    function setToolbar(domId) {
      if (domId === '__ai__') {
        renderToolbar(`
          <span style="font-size:11px;color:#6B4F1F;font-weight:700">🤖 AI 知識整理助手</span>
        ...
        renderToolbar(`
          <span style="font-size:11px;color:#1F1A14">wiki/ — ${cnt} 個文件</span>
          <button class="wiki-ai-btn cancel" style="margin-left:auto;padding:4px 10px"
      ...
      window.mWikiOpen = async (i) => {
        ...
        try {
          ...
          $('m-detail-body').innerHTML = `
            <div class="m-detail-desc" style="color:#1F1A14 !important">
              ${(d.frontmatter?.description || d.content.slice(0,120)).replace(/</g,'&lt;')}
            </div>
            <div style="margin-top:8px;padding:10px;background:rgba(120,84,40,0.12);border-radius:6px;max-height:240px;overflow-y:auto">
              <pre style="font-size:10px;color:#1F1A14 !important;white-space:pre-wrap;word-break:break-all;line-height:1.55">
${d.content.replace(/</g,'&lt;').slice(0,1000)}</pre>
      ...
      const chatHTML = aiHistory.length === 0
        ? `<div class="wiki-empty-ai">
            ...
            <div class="wiki-empty-ai-text">
              ...
              <br><br>
              <span style="color:#4A4030">範例：整理 docs/ 裡所有關於 TypeScript 的文件</span>
            </div>
          </div>`
        : aiHistory.map(h => {
            if (h._type === 'files') {
    ...
    window.mAiSend = async () => {
      ...
      if (chat) {
        ...
        loading.innerHTML = '<span style="color:#3A3228 !important">AI 思考中…</span>';
