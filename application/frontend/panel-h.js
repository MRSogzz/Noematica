/* ============================================================
   LLM WIKI — Panel H  AI 知識整理助手（原本掛在 M 鍵，現在搬到 H 鍵）
   讀取 wiki/，透過後端 /api/wiki/generate 呼叫本地或雲端 AI
   ============================================================ */

// 注入到 PANELS（在 hud-panels.js 定義的 PANELS 物件上擴充）
// 內部函式/全域變數保留 m 前綴（mAiSend、mWikiOpen…），純粹是搬遷後的歷史命名，
// 跟現在掛在哪個按鍵無關，不影響功能。
PANELS.h = async function h() {
    // ── 狀態 ──
...
// ── M：認知地圖（3D 卡片關聯地圖）──────────────────────────────────────────────
// 實際內容是 panel-m-map.html，由 hud-core.js 的 openModal() 在 fullwidth 模式下
// 用 iframe 注入（跟 F3 測試面板走同一套機制），這裡只需要一個 no-op 讓
// openModal() 結尾的 PANELS[id]() 呼叫不會出錯。
PANELS.m = async function () {};
