...
## 1. 尺寸：完全沿用 hud.css 的 .modal 基礎標準，不覆寫

```css
/* hud.css 的 .modal（M 認知地圖直接用這個，沒有額外皮膚） */
.modal {
...
`panel-skins.css` 對 B/H/F1/F2/F3/F4/F5 這七個面板**完全沒有另外寫 width/height**——不管是固定 px 還是 `clamp()`，一律不覆寫，直接沿用上面這個基礎規則。這樣「統一標準」不是靠每個面板各自訂一套數字再對齊，而是**所有面板從頭到尾用的就是同一條 CSS 規則**，之後 `hud.css` 這條規則如果調整，全部面板會一起變，不會有好幾套標準各自維護、容易兜不起來的問題。

側欄（`.m-sidebar`，80px）、詳情欄（`.m-detail`，280px）也一樣——不再另外收窄，直接沿用 `hud.css` 的預設寬度，全部面板一致。
...
|---|---|---|
| 尺寸 | `hud.css` 的 `.modal` 基礎尺寸（§1，自始至終沒變過） | 不變 |
| 邊框/底色 | `hud.css` 預設深色科技風 | 皮膚配色（`--nw-body`／`--nw-trim-*`，可由 theme.json 覆寫） |
| 標題列 | 正常顯示，跟 M 同樣大小、同樣位置 | 只換底色/文字色，位置大小不變 |
| 關閉鈕 | `hud.css` 預設圓形按鈕＋「✕」文字，跟 M 同一個元件、同一個位置 | 只換視覺（顯示圖示），位置不變 |
| 四個角落裝飾 | 完全不存在（`display:none`，不佔版面空間） | 顯示徽章／裝飾邊角 |
| B/F4 的道具格／里程碑卡、F1/F2/F5 清單 | 跟 M 一樣是預設深色方格 | B/F4 換成對應底圖/上色；F1/F2/F5 換成單欄清單、邊緣透明 |
| 文字顏色 | `hud.css` 預設淺色（給深色底用） | `rgb(45,30,27)` 咖啡色（或 theme.json 指定的顏色） |

**標題不會太小、內容區不會太小**：因為兩種狀態下都是同一份 `hud.css` 標題/內容區樣式在跑，沒有另外覆寫過字級或版面尺寸，這正是「全部面板統一標準」在視覺上的直接結果。
...
- **右上（關閉鈕）**：**沒有獨立的 `.nw-corner`**——直接沿用 `hud.css` 原本在 `.modal-header` 裡的 `.modal-close`，跟 M 用同一個元件、同一個位置（`margin-left:auto` 讓它自然貼齊 header 右側），套用風格後只換視覺，不換位置。這是跟第三版的差異：第三版把關閉鈕也做成一個獨立 `.nw-corner-tr` 疊在角落，這一版拿掉了，因為跟「關閉鈕位置要統一標準」衝突。
...
| F3（測試套件監控，素材前綴 t_） | `t_badge.png`（缺件，見 NAL.md §7） | 同上 | `#eef1f5` |
| F4（目標專案時程，素材前綴 p_） | `p_badge.png`（缺件） | 同上 | `#f5ecd9` |
| F5（知識圖鑑，素材前綴 w_） | `w_badge.png`（缺件） | 同上 | `#f2ede0` |

尺寸：全部面板共用同一條 `hud.css` 規則，見 §1，這裡不重複列。
...
- **Grid Layout**（B／F4）：套用風格前後都是方格，B 的底圖從 `hud.css` 預設換成 `b_slot.png`；F4（里程碑卡）沒有專屬底圖素材，套用後只重新上色，不換版面結構。
- **List Layout**（F1／F2／F5，靠 `.nw-list` 標記 class 啟用）：沒套用風格時是 `hud.css` 預設方格（跟 M 一樣），套用後換成單欄橫列、邊緣透明。F1 面板開啟時預設直接進入今天的筆記編輯畫面（見 `hud-panels.js` 的 `f1()`）；F5 是文件搜尋結果清單。
- **Chat Layout**（H）：`panel-h.js` 完全自訂，不套用上面兩種骨架，也不受 `.nw-skins-enabled` 影響（聊天室內容本來就是動態資料，不算這份檔案的皮膚系統範圍）。
- **Custom/iframe Layout**（F3）：內容整個是 `panel-tests.html` 的 iframe（見 `hud-core.js` openModal() 的 fullwidth 分支），不套用 Grid/List Layout，也不需要 `.m-item` 相關的顏色覆寫，只有外層邊框/角落/標題列會套用皮膚。

---

## 6. 隔離：確保不影響其他面板（M）

- `.nw-frame`／`.nw-list`／`.modal-skin-<id>` 三個 class 都只在 `hud-core.js` 的 `openModal()` 裡、id 屬於 `b`/`h`/`f1`/`f2`/`f3`/`f4`/`f5` 時才會掛上，M 永遠不會有這幾個 class；`.nw-skins-enabled` 是掛在 `<html>` 上的全域開關，由 `applyTheme()` 依目前套用的主題決定，跟個別 `.modal` 無關。
- `panel-skins.css` 裡沒有任何一條規則是寫裸的 `.modal { ... }`，每一條都至少掛在 `.modal-skin-*` 或 `.modal.nw-frame` 底下。
- 尺寸（§1）跟 M 共用同一條 `hud.css` 規則，不是「各自訂一套數字」，物理上不可能不同步。
- `panelSkins` 沒有被目前主題定義時（§2），B/H/F1/F2/F3/F4/F5 全部維持「跟 M 一樣的深色科技風」，不會出現半套皮膚或版面跑掉的中間態，也不會反過來影響到 M 本身（M 從頭到尾沒有被這套機制碰過，永遠不會有 `.nw-frame`/`.modal-skin-*` class）。

---

## 7. 新增／修改一個 `.modal-skin-*` 時的檢查清單

- [ ] **不要覆寫 `.modal` 的 width/height**——直接沿用 `hud.css` 的 92vw/90vh，這是全部面板統一標準的關鍵
